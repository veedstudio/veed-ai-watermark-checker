...
To install straight from git (e.g. in a deployment image), pin a tag and request the extra
(VideoSeal itself still needs the `--no-deps` step above):

```bash
pip install "veed-videoseal[videoseal] @ git+https://github.com/veedstudio/ai-content-watermarking@v0.5.0"
